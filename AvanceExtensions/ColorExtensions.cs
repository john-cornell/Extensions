...
namespace Avance
{
    public static class ColorExtensions
    {
        ...
        public static Color GetContrast(this Color me, bool preserveOpacity)
        {
            Color inputColor = me;
            ...
            byte avgColorValue = (byte)((me.R + me.G + me.B) / 3);
            int diff_r = Math.Abs(me.R - avgColorValue);
            int diff_g = Math.Abs(me.G - avgColorValue);
            int diff_b = Math.Abs(me.B - avgColorValue);
            if (diff_r < 20 && diff_g < 20 && diff_b < 20) //The color is a shade of gray
            {
                if (avgColorValue < 123) //color is dark
                {
                    inputColor = Color.FromArgb(me.A, 220, 230, 50);
                }
                else
                {
                    inputColor = Color.FromArgb(me.A, 255, 255, 50);
                }
            }
            byte sourceAlphaValue = me.A;
            if (!preserveOpacity)
            {
                sourceAlphaValue = Math.Max(me.A, (byte)127); //We don't want contrast color to be more than 50% transparent ever.
            }
